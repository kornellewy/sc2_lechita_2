...
GATEWAY, CYBERNETICSCORE, STALKER, VOIDRAY, STARGATE, ZEALOT, OBSERVER, \
ROBOTICSFACILITY
...
class sc2_lechita_2(sc2.BotAI):
    def __init__(self, use_model = False):
        ...
        if self.use_model:
            name = "BasicCNN-30-epochs-0.0001-LR-4.2"
            print("uning model: ", name)
    ...
    async def intel(self):
        ...
        # we draw ouer unit and buildings
        draw_dict = {
                        NEXUS: [15, (0,255,0)],
                        PYLON: [3, (20,235,0)],
                        PROBE: [1, (55,200,0)],
        ...
        for enemy_unit in self.known_enemy_units:
            ...
            # we check if its not building
            if not enemy_unit.is_structure:
                pos = enemy_unit.position
                # we check if it is worker
                if enemy_unit.name.lower() is worker_names:
                    cv2.circle(game_data, (int(pos[0]), int(pos[1])), 1, (55, 0, 155), -1)
                else:
                    cv2.circle(game_data, (int(pos[0]), int(pos[1])), 3, (50, 0, 215), -1)
        # we use OBSERVERs for scouting
        for observer in self.units(OBSERVER).ready:
            pos = observer.position
            cv2.circle(game_data, (int(pos[0]), int(pos[1])), 1, (255, 255, 255), -1)
        # display stacts about the game ala normalization
        line_max = 50
        ...
        if not HEADLESS:
            resized = cv2.resize(self.flipped, dsize = None, fx=4, fy=4)
            # we start window and draw info
    ...
    async def attack(self):
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            if self.iteration > self.do_something_after:
                # using the model
                if self.use_model:
                    prediction = self.model.prediction([self.flipped.reshape([-1, 176, 200, 3])])
...
run_game(maps.get("Abyssal reef LE"), [
    Bot(Race.Protoss, sc2_lechita_2(use_model=True)),
